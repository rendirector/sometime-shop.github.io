...
function cooldown()
	{
		...
		if($lastclickcooldown > time()-(3600)) //ตัวเลขคือจำนวนวินาที
		{
			return false;
		}else{
			return true;
...
if (isset($_POST['random'])) {
	if(cooldown() == true or $lastclickcooldown > time())
		{	
			...
			$filename =  'stock/nfa.txt'; //ชื่อไฟล์ที่ใส่ไอดีแท้ไว้ email:pass
			$data = rmtxt($filename);
...
<div class="col-sm-6 mx-auto">
  <div class="card">
	...
	<img class="imgphone" src="https://media.discordapp.net/attachments/895295408413216778/932135017910505532/FREE.gif">	
	
	<center>	
	  <br />
	  
	  <?php if(cooldown() == true or $lastclickcooldown > time()) { ?>
	  <button 
      type="submit"
	  name="random"
	  id="random"
	  ...
	  <?php }else{ ?>
		<div class="row">
			<div class="col-sm-11">
				<button 
					type="submit"
					class="btn btn-block btn-danger me-3 w-100 mb-5 text-center animate__animated animate__headShake" disabled>
					<i class="fas fa-hourglass-half"></i> กรุณารอ cooldown สักครู่ !
				</button>	
	 		</div>
			<div class="col-sm-1">
			<button 
				type="submit"
				class="btn btn-block btn-info me-3 w-100 mb-5 text-center" onclick="location.href='random'"><i class="fas fa-sync-alt"></i></button>		
			</div>			
	  </div>
	  <?php } ?>
	</center>	
